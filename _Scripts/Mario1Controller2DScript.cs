...
public class Mario1Controller2DScript : MonoBehaviour 
{
	...
	void Update () {
		...
		if (Input.GetKeyUp(KeyCode.W) || Time.time - jumpTimer > maxExtraJumpTime){
	...
	void FixedUpdate (){
		...
		if (Input.GetKeyDown(KeyCode.L) && Time.time - sprintTimer > sprintDelay && isGrounded || jumpedDuringSprint){
			//... then sprint
			Player.velocity = new Vector2(Input.GetAxis("Horizontal") * moveSpeed * Time.deltaTime * sprintMultiplier,Player.velocity.y);
		...
		if (playerJumped){
			Player.AddForce(new Vector2(0,initialJumpForce)); //"Jump" our player up in the air!
		...
		if (playerJumping && Time.time - jumpTimer > delayToExtraJumpForce){
			Player.AddForce(new Vector2(0,extraJumpForce)); //... then add some additional force to the jump
		}
	}
}
